...
#check for artifacts to be downloaded to a pipeline volume
if ssh ${SSH_USERNAME}@${HOST_ADDRESS} -p ${SSH_PORT} '[ -d ~/volume/output ]'
then
{
echo "Uploading artifacts to /codefresh/volume/ ..."
rsync  -Pzare "ssh -p${SSH_PORT} -o LogLevel=ERROR"  ${SSH_USERNAME}@${HOST_ADDRESS}:~/volume/output  /codefresh/volume/
}
fi
